...
class LTXPlusGenerate:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                # Frame injection
                "key_frames":      ("IMAGE", {"tooltip": "Animated image (WebP/GIF) or image batch containing all keyframes."}),
                "key_frame_poses": ("STRING", {"default": "0", "tooltip": "Comma-separated frame indices where each keyframe should be placed (e.g. '0,25,50')."}),
                "guide_type":      (["guide", "inject"], {"default": "guide", "tooltip": "guide = LTXVAddGuide (append + RoPE), inject = frame injection (inplace replace)."}),
                "guide_strength":  ("FLOAT", {"default": 0.7, "min": 0.0, "max": 1.0, "step": 0.01}),
        ...
        **kwargs,
        ...
        try:
            result = self._generate_impl(
                ...
                key_frames=key_frames, key_frame_poses=key_frame_poses,
                guide_type=guide_type, guide_strength=guide_strength, crf=crf,
    ...
    def _generate_impl(
        ...
        key_frames, key_frame_poses, guide_type, guide_strength, crf,
        audio, audio_vae,
        ...
        has_image_guides = False

        if key_frames is not None and key_frame_poses:
            has_image_guides = True
            # Parse frame positions
            poses = [int(x.strip()) for x in key_frame_poses.split(",") if x.strip()]

            scale_factors = vae.downscale_index_formula
            time_sf = scale_factors[0]
            _, _, latent_length, latent_height, latent_width = latent_dict["samples"].shape

            # Split key_frames batch into individual frames
            num_kf = key_frames.shape[0]
            frames_list = [key_frames[i:i+1] for i in range(min(num_kf, len(poses)))]

            if guide_type == "guide":
                # Guide mode: use LTXVAddGuide.append_keyframe (appends to latent + RoPE)
                for fi, (frame, frame_idx) in enumerate(zip(frames_list, poses)):
                    # CRF compression if enabled
                    if crf > 0:
                        import comfy_extras.nodes_lt as lt_nodes
                        if hasattr(lt_nodes, 'encode_single_frame'):
                            import io as _io
                            import numpy as np
                            frame_np = (frame[0].cpu().numpy() * 255).astype(np.uint8)
                            buf = _io.BytesIO()
                            lt_nodes.encode_single_frame(buf, frame_np, crf)
                            buf.seek(0)
                            import av
                            container = av.open(buf)
                            for av_frame in container.decode(video=0):
                                frame = torch.from_numpy(
                                    av_frame.to_ndarray(format="rgb24").astype(np.float32) / 255.0
                                ).unsqueeze(0)
                                break
                            container.close()

                    _, t = LTXVAddGuide.encode(vae, latent_width, latent_height, frame, scale_factors)
                    frame_idx_actual, latent_idx = LTXVAddGuide.get_latent_index(
                        positive, latent_length, len(frame), frame_idx, scale_factors
                    )
                    logger.info(f"Guide keyframe {fi}: frame_idx={frame_idx_actual}, latent_idx={latent_idx}, strength={guide_strength}")
                    ...
                    positive, negative, latent_samples, noise_mask = LTXVAddGuide.append_keyframe(
                        ...
                        t, guide_strength, scale_factors,
                    )
                    latent_dict = {"samples": latent_samples, "noise_mask": noise_mask}

            elif guide_type == "inject":
                # Inject mode: inplace replace latent frames (like KJNodes LTXVImgToVideoInplaceKJ)
                samples = latent_dict["samples"].clone()
                for fi, (frame, frame_idx) in enumerate(zip(frames_list, poses)):
                    # Handle negative indices
                    total_pixel_frames = (latent_length - 1) * time_sf + 1
                    if frame_idx < 0:
                    ...
                    if frame_idx == 0:
                        ...
                        lat_idx = (frame_idx - 1) // time_sf + 1
                    lat_idx = min(lat_idx, latent_length - 1)

                    # Encode and replace
                    pixels = comfy.utils.common_upscale(
                        frame.movedim(-1, 1), latent_width * scale_factors[2], latent_height * scale_factors[1],
                        "bilinear", "center"
                    ).movedim(1, -1)
                    t = vae.encode(pixels[:, :, :, :3])

                    cond_length = t.shape[2]
                    end_idx = min(lat_idx + cond_length, latent_length)
                    actual_length = end_idx - lat_idx

                    samples[:, :, lat_idx:end_idx] = t[:, :, :actual_length]
                    noise_mask[:, :, lat_idx:end_idx] = 1.0 - guide_strength

                    logger.info(f"Inject keyframe {fi}: pixel_idx={frame_idx}, latent_idx={lat_idx}, strength={guide_strength}")

                latent_dict = {"samples": samples, "noise_mask": noise_mask}

        # Propagate keyframe_idxs to external guider (guide mode only)
        if has_image_guides and guide_type == "guide" and guider is not None:
            ...
            if kf_idxs is not None:
                ...
                guider_pos = node_helpers.conditioning_set_values(guider_pos, {"keyframe_idxs": kf_idxs})
                guider_neg = node_helpers.conditioning_set_values(guider_neg, {"keyframe_idxs": kf_idxs})
                guider.inner_set_conds({"positive": guider_pos, "negative": guider_neg})
        ...
        if guider is None:
            from .multimodal_guider import MultimodalGuider
        ...
        if do_upscale and (do_spatial_upscale or do_temporal_upscale):
            ...
            try:
                ...
                for _rediff_i in range(_rediff_passes):
                    ...
                    # Re-inject keyframes at upscaled resolution
                    up_noise_mask = None

                    if key_frames is not None and key_frame_poses and guide_type == "guide":
                        ...
                        for fi, (frame, frame_idx) in enumerate(zip(up_frames_list, up_poses)):
                            ...
                            if crf > 0:
                                ...
                                if hasattr(lt_nodes, 'encode_single_frame'):
                                    import io as _io
                                    import numpy as np
                                    frame_np = (up_frame[0].cpu().numpy() * 255).astype(np.uint8)
                                    buf = _io.BytesIO()
                                    lt_nodes.encode_single_frame(buf, frame_np, crf)
                                    buf.seek(0)
                                    import av
                                    container = av.open(buf)
                                    for av_frame in container.decode(video=0):
                                        up_frame = torch.from_numpy(
                                            av_frame.to_ndarray(format="rgb24").astype(np.float32) / 255.0
                                        ).unsqueeze(0)
                                        break
                                    container.close()

                            _, up_t = UpGuide.encode(vae, latent_width, latent_height, up_frame, up_scale_factors)

                            frame_idx_actual, latent_idx = UpGuide.get_latent_index(
                                positive, latent_length, len(up_frame), frame_idx, up_scale_factors
                            )

                            logger.info(f"Upscale re-inject keyframe {fi}: frame_idx={frame_idx_actual}, latent_idx={latent_idx}, strength={guide_strength}")

                            positive, negative, up_latent_samples, up_noise_mask = UpGuide.append_keyframe(
                                positive, negative, frame_idx_actual,
                                up_latent_dict["samples"], up_noise_mask,
                                up_t, guide_strength, up_scale_factors,
                            )
                        ...
                        # I2V + upscale path: main pass pinned the keyframe
                        # via inplace replace + noise_mask. After spatial
                        # or temporal upscale, that frame is just a blurry
                        # upsampled copy, and without a noise_mask the
                        # rediffusion pass will denoise it away.
                        # Re-inject at the new (upscaled) resolution with
                        # the same inject semantics.
                        ...
                        upsampled, up_noise_mask = self._inject_keyframes_at_latent_res(
                            upsampled, key_frames, rd_poses,
                            guide_strength, up_scale_factors, vae,
                            label="Upscale",
                        )
                        output_latent = {"samples": upsampled}
                ...
                if _iclora_needs_normal_upscale:
                    ...
                    if key_frames is not None and key_frame_poses and guide_type == "guide":
                        ...
                        p3_poses = [int(x.strip()) for x in key_frame_poses.split(",") if x.strip()]
                        p3_num_kf = key_frames.shape[0]
                        p3_frames_list = [key_frames[i:i+1] for i in range(min(p3_num_kf, len(p3_poses)))]

                        for fi, (img, frame_idx) in enumerate(zip(p3_frames_list, p3_poses)):
                            _, _, latent_length, latent_height, latent_width = up_latent_dict["samples"].shape
                            ...
                            logger.info(f"Pass 3 re-inject keyframe {fi}: frame_idx={frame_idx_actual}, latent_idx={latent_idx}, strength={guide_strength}")
                            positive, negative, up_latent_samples, up_noise_mask = UpGuide.append_keyframe(
                                positive, negative, frame_idx_actual,
                                up_latent_dict["samples"], up_noise_mask,
                                up_t, guide_strength, up_scale_factors,
                        ...
                        # Inject-mode parallel of the guide branch above.
                        # Pass 3 is after full-resolution spatial upscale,
                        # so poses are already in full-frame units (no
                        # temporal doubling needed here — different from
                        # site 1's temporal-only path).
                        p3_poses = [int(x.strip()) for x in key_frame_poses.split(",") if x.strip()]
                        upsampled, up_noise_mask = self._inject_keyframes_at_latent_res(
                            upsampled, key_frames, p3_poses,
                            guide_strength, up_scale_factors, vae,
                            label="Pass 3",
                        )
                        output_latent = {"samples": upsampled}
                    ...
                    # --- Normal re-diffusion ---
                    # Run rediffusion whenever we have keyframes of ANY
                    # mode (guide OR inject). Previously this was gated
                    # on guide mode only, so inject-mode users silently
                    # got no refinement after the spatial upscale.
                    p3_has_image_guides = (key_frames is not None and key_frame_poses
                                            and guide_type in ("guide", "inject"))
        ...
        if not do_upscale and rediffusion_mask is not None and upscale_denoise > 0 and upscale_steps > 0:
            ...
            if key_frames is not None and key_frame_poses and guide_type == "guide":
                ...
                for fi, (guide_entry, frame_idx) in enumerate(zip(rd_frames_list, rd_poses)):
                    ...
                    frame_idx_actual, latent_idx = RdGuide.get_latent_index(
                        positive, latent_length, len(guide_entry), frame_idx, rd_scale_factors
                    )

                    logger.info(f"Masked rediff guide keyframe {fi}: frame_idx={frame_idx_actual}, latent_idx={latent_idx}")
                    ...
                    positive, negative, rd_latent_samples, rd_guide_noise_mask = RdGuide.append_keyframe(
                        ...
                        rd_t, guide_strength, rd_scale_factors,
                    )
                ...
                # Inject-mode parallel for masked rediffusion. No spatial
                # upscale happened here (this is a non-upscale path), so
                # rd_latent already has the full generation resolution.
                # Re-inject at that resolution and then fold the user's
                # rediffusion_mask into the resulting noise_mask.
                rd_scale_factors = vae.downscale_index_formula
                rd_poses = [int(x.strip()) for x in key_frame_poses.split(",") if x.strip()]

                rd_latent, rd_guide_noise_mask = self._inject_keyframes_at_latent_res(
                    rd_latent, key_frames, rd_poses,
                    guide_strength, rd_scale_factors, vae,
                    label="Masked rediff",
                )
                _, _, _, rd_lat_h, rd_lat_w = rd_latent.shape
                rd_noise_mask = self._apply_rediffusion_mask(
                    rd_guide_noise_mask, rediffusion_mask,
                    rediffusion_mask_strength, rd_lat_h, rd_lat_w,
                )
    ...
    @staticmethod
    def _inject_keyframes_at_latent_res(samples, key_frames, poses_list,
                                        guide_strength, scale_factors, vae,
                                        label=""):
        """Re-inject keyframes (guide_type='inject' semantics) at whatever
        latent spatial/temporal resolution `samples` currently has.

        For each pose, resize the keyframe pixels to match the current
        latent's pixel dims, VAE-encode, inplace-replace the target latent
        frames, and build a noise_mask entry that preserves them during
        subsequent sampling (mask = 1-guide_strength on keyframe slots,
        1.0 elsewhere).

        Returns (new_samples, noise_mask). new_samples is a clone of
        `samples` with keyframes written in. noise_mask has shape
        [B, 1, T, H, W] matching samples.

        Used by all three rediffusion paths (main upscale, IC-LoRA
        pass 3, masked rediffusion) so the I2V keyframe stays pinned
        ...
        num_kf = key_frames.shape[0]
        frames_list = [key_frames[i:i+1] for i in range(min(num_kf, len(poses_list)))]

        samples_mut = samples.clone()
        noise_mask = torch.ones(
            samples_mut.shape[0], 1, samples_mut.shape[2],
            samples_mut.shape[3], samples_mut.shape[4],
            device=samples_mut.device, dtype=samples_mut.dtype,
        )

        prefix = f"{label} " if label else ""

        for fi, (frame, frame_idx) in enumerate(zip(frames_list, poses_list)):
            # Handle negative indices (-1 = last pixel frame)
            total_pixel_frames = (T - 1) * time_sf + 1
            if frame_idx < 0:
                frame_idx = max(0, total_pixel_frames + frame_idx)
            ...
            if frame_idx == 0:
                ...
                lat_idx = (frame_idx - 1) // time_sf + 1
            lat_idx = min(lat_idx, T - 1)

            # Resize to current latent's pixel resolution, then encode
            pixels = _cu.common_upscale(
                frame.movedim(-1, 1),
                W * scale_factors[2],
                H * scale_factors[1],
                "bilinear", "center",
            ...
            end_idx = min(lat_idx + cond_length, T)
            actual_length = end_idx - lat_idx

            samples_mut[:, :, lat_idx:end_idx] = t_kf[:, :, :actual_length]
            noise_mask[:, :, lat_idx:end_idx] = 1.0 - guide_strength

            logger.info(
                f"{prefix}Re-inject (inject mode) keyframe {fi}: "
                f"pixel_idx={frame_idx}, latent_idx={lat_idx}, "
                f"strength={guide_strength}"
            )

        return samples_mut, noise_mask
    ...
    def _rebuild_iclora_guider(up_model, positive, negative, vae,
        ...
        """
        from .multimodal_guider import ICLoRAGuider
...
NODE_CLASS_MAPPINGS = {"LTXPlusGenerate": LTXPlusGenerate}
NODE_DISPLAY_NAME_MAPPINGS = {"LTXPlusGenerate": "\U0001f15b\U0001f163\U0001f167 LTX Plus Generate"}
